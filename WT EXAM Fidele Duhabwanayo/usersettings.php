...
<section> 
  ...
  <form method="post" action="usersettings.php">
    ...
    <input type="number" id="SettingID" name="SettingID"><br><br>

    <label for="UserID">UserID</label>
    <input type="text" id="UserID" name="UserID" required><br><br>

    <label for="SettingName">SettingName</label>
    <input type="text" id="SettingName" name="SettingName" required><br><br>

    <label for="SettingValue">SettingValue</label>
    <input type="text" id="SettingValue" name="SettingValue" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO usersettings(SettingID, UserID, SettingName, SettingValue) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("isss", $SettingID,$UserID, $SettingName, $SettingValue);
...
// SQL query to fetch data from the usersettings
...
<head>
    ...
    <title>Detail information of User setting</title>
    <style>
        table {
            width: 80%;
...
<body><center>
    <center><h1><u>User Setting Table</u></h1></center>
    <table border="2">
        <tr>
            <th>SettingID</th>
            <th>UserID</th>
        ...
        if ($result === false) {
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    $SettingID = $row['SettingID']; // Fetch the SettingID
                    echo "<tr>
                        <td>" . $row['SettingID'] . "</td>
                        <td>" . $row['UserID'] . "</td>
                        ...
                        <td><a style='padding:4px' href='Delete_Usersettings.php?SettingID=$SettingID'>Delete</a></td> 
                        <td><a style='padding:4px' href='Update_usersettings.php?SettingID=$SettingID'>Update</a></td> 
                    </tr>";
                }
            } else {
